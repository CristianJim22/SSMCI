      ...
      <div class="content-wrapper">
            ...
            <div class="card" style="overflow-x: scroll;">
               ...
               <div class="card-body">
                  ...
                  <div class="row d-flex justify-content-center">
                     <div class="col-lg-8" data-toggle="modal" data-target="#exampleModal2" id="buttonInforme">
                     </div>
         ...
         <div class="modal fade" id="exampleModal2" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel2" aria-hidden="true">
            <div class="modal-dialog" role="document">
               <div class="modal-content">
                  <div class="modal-header">
                     <h5 class="modal-title" id="exampleModalLabel2">Filtro para el informe de productividad</h5>
                     <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                     </button>
                  </div>
                  <form id="solicitudFiltro" autocomplete="off" method="post">
                     <div class="modal-body form-group">
                        <div class="row">
                           <div class="col-lg-6">
                              <label>Desde:</label>
                              <input type="date" class="form-control form-control-sm" id="desdeFiltro" required>
                           </div>
                           <div class="col-lg-6">
                              <label>Hasta</label>
                              <input type="date" class="form-control form-control-sm" id="hastaFiltro" required>
                           </div>
                        </div>
                        <div class="row mt-3">
                           <div class="col-lg-12">
                     ...
                     <div class="modal-footer">
                        <button type="submit" class="btn btn-primary" id="guardarFiltro">Guardar</button>
                     </div>
                  </form>
               </div>
            </div>
...
<script type="text/javascript">
   ...
   $("#solicitudFiltro").submit(function(e) {
      e.preventDefault();
      $('#guardarFiltro').prop('disabled', true);
      var desde = $('#desdeFiltro').val();
      var hasta = $('#hastaFiltro').val();
      var idPeriodo = $('#periodos').val();

      if (desde && hasta && idPeriodo) {
         $.ajax({
            url: 'descargarInforme.php',
            type: 'POST',
            data: {
               desde: desde,
               hasta: hasta,
            ...
            success: function(response) {
               ...
               link.download = "informe_productividad.xlsx";
               document.body.appendChild(link);
               link.click();
               document.body.removeChild(link);
               window.URL.revokeObjectURL(link.href);

               $('#guardarFiltro').prop('disabled', false);
   ...
   $('#exampleModal2').on('hidden.bs.modal', function() {
      $('#desdeFiltro').val('');
      $('#hastaFiltro').val('');
      $('#mensajeUsuario2').removeClass().hide();
   });
            ...
            if (data.state) {
               ...
               datos.forEach(function(registro, index) {
                  ...
                  var myChart = new Chart(ctx, {
                     ...
                     type: 'pie',
                     data: {
                        labels: ['Completado', 'Equipo nuevo con mantenimiento completado', 'Sin mantenimiento'],
                        datasets: [{
                           label: 'Dataset 1',
   ...
   function printtable(datos) {
      ...
      $('#buttonInforme').empty();
      var tabla = "";
      $('#buttonInforme').append('<button class="btn btn-sm btn-primary float-right mb-2" id="informe">INFORME DE PRODUCTIVIDAD<i class="fas fa-download"></i></button>');
      ...
      tabla = '<table class="font-12 table-hover" style="max-width:100% !important; border: 1px solid;"><thead><tr><th style="width:5.6%; border: 1px solid;">UNIDAD</th><th style="width:5.6%; border: 1px solid;">TOTAL DE EQUIPOS</th><th style="width:5.6%; border: 1px solid;">EQUIPOS NUEVOS CON MANTENIMIENTO REALIZADO</th><th style="width:5.6%; border: 1px solid;">EQUIPOS CON MANTENIMIENTO REALIZADO</th><th style="width:5.6%; border: 1px solid;">EQUIPOS PENDIENTES DE MANTENIMIENTO</th><th style="width:5.6%; border: 1px solid;">ENE</th><th style="width:5.6%; border: 1px solid;">FEB</th><th style="width:5.6%; border: 1px solid;">MAR</th><th style="width:5.6%; border: 1px solid;">ABR</th><th style="width:5.6%; border: 1px solid;">MAY</th><th style="width:5.6%; border: 1px solid;">JUN</th><th style="width:5.6%; border: 1px solid;">JUL</th><th style="width:5.6%; border: 1px solid;">AGO</th><th style="width:5.6%; border: 1px solid;">SEP</th><th style="width:5.6%; border: 1px solid;">OCT</th><th style="width:5.6%; border: 1px solid;">NOV</th><th style="width:5.6%; border: 1px solid;">DIC</th></tr></thead><tbody>';
      datos.forEach(function(registro, index) {
         tabla += '<tr><td style="border: 1px solid;">' + registro.unidad + '</td><td style="border: 1px solid;">' + registro.total + '</td><td style="border: 1px solid;">' + registro.congarantia + '</td><td style="border: 1px solid;">' + registro.completado + '</td><td style="border: 1px solid;">' + registro.pendiente + '</td><td style="border: 1px solid;">' + registro.ene + '</td><td style="border: 1px solid;">' + registro.feb + '</td><td style="border: 1px solid;">' + registro.mar + '</td><td style="border: 1px solid;">' + registro.abr + '</td><td style="border: 1px solid;">' + registro.may + '</td><td style="border: 1px solid;">' + registro.jun + '</td><td style="border: 1px solid;">' + registro.jul + '</td><td style="border: 1px solid;">' + registro.ago + '</td><td style="border: 1px solid;">' + registro.sep + '</td><td style="border: 1px solid;">' + registro.oct + '</td><td style="border: 1px solid;">' + registro.nov + '</td><td style="border: 1px solid;">' + registro.dic + '</td></tr>';
      });
      tabla += '</tbody></table>';
   ...
   $(document).on('click', '#descargar', function() {
      ...
      $('#invoice').printThis({
         header: '<table style="width: 100%;" class="mb-4"><tr><td><div id="logo"><img src="/SSMCI/dist/img/imsslogo-grey.jpg"  width= "70" height="75"></div></td><td>O.O.A.D AGUASCALIENTES<br>COORDINACIÓN DELEGACIONAL DE INFORMÁTICA<br>AVANCE DE MANTENIMIENTOS A EQUIPOS  DE CÓMPUTO<br>PERIODO:' + periodo + ' <i id="periodoPrint"></i></td></tr></table>',
