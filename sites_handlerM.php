...
if(isset($_GET['periodo']) && isset($_GET['df'])){
   ...
   $db = new Databaseocs();
	/*A*/
   $query = $db->connect()->prepare('SELECT id FROM sites_mdf1 WHERE df = :df and periodo = :periodo');
   $query->execute(['df' => $df,'periodo' => $periodo]);
   $row = $query->fetch(PDO::FETCH_NUM);
   $rowsA = $query->rowCount();
   if($rowsA == 0){
      header('location: sites-mdf1.php?p='.$periodo.'&df='.$df);
      exit;
      //echo "MDF1";
   }

   /*B*/
   $query = $db->connect()->prepare('SELECT id FROM sites_mdf2 WHERE df = :df and periodo = :periodo');
   $query->execute(['df' => $df,'periodo' => $periodo]);
   $row = $query->fetch(PDO::FETCH_NUM);
   $rowsB = $query->rowCount();
   if($rowsB == 0){
      header('location: sites-mdf2.php?p='.$periodo.'&df='.$df);
      exit;
      //echo "MDF2";
   }

   /*C*/
   $query = $db->connect()->prepare('SELECT id FROM sites_mdf3 WHERE df = :df and periodo = :periodo');
   $query->execute(['df' => $df,'periodo' => $periodo]);
   $row = $query->fetch(PDO::FETCH_NUM);
   $rowsC = $query->rowCount();
   if($rowsC == 0){
      header('location: sites-mdf3.php?p='.$periodo.'&df='.$df);
      exit;
      //echo "MDF2";
   }
